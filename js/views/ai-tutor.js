...
App.views.aiTutor = {
    ...
    renderExpertCards: function() {
        ...
        aiTutorExpertGroup.innerHTML = Object.values(App.data.aiExpertsData).map((expert) => {
    ...
    renderTutorCategories: function() {
        ...
        const categories = (App.translations[App.state.currentLang].aiTutor.categories || App.translations['en'].aiTutor.categories);
        aiTutorCategoryGroup.innerHTML = Object.entries(categories).map((entry) => {
            const key = entry[0];
            const value = entry[1];
    ...
    getAdviceOrDiagnosis: async function(isDoctor, followUpText) {
        ...
        const chatHistory = App.state.aiTutorChatHistory;
        ...
        const expertGroup = document.getElementById('ai-tutor-expert-group');
        const button = document.getElementById('get-advice-btn');
        const errorEl = document.getElementById('ai-tutor-error-message');
        ...
        try {
            const expertData = App.data.aiExpertsData[currentExpertId];
            if (!expertData) {
            ...
            if (followUpQuestions && followUpQuestions.length > 0) {
                const expertName = expertData.name[App.state.currentLang] || expertData.name['en'];
                const followUpHTML = ''
